...
voiceButton.addEventListener('click', () => {
    speakDirections("Listening for a command...");
    recognition.start();
});
...
recognition.addEventListener('result', (event) => {
    ...
    // Match voice command to destination options
    for (const location in campusLocations) {
        if (command.includes(location.toLowerCase())) {
    ...
    const errorText = "Sorry, I didn't understand the destination. Please try again.";
    directionsDiv.innerHTML = `<p>${errorText}</p>`;
    speakDirections(errorText);
});

recognition.addEventListener('error', (event) => {
    const errorText = "An error occurred while processing your command. Please try again.";
...
document.getElementById('mute-btn').addEventListener('click', () => {
    ...
    muteButton.textContent = isMuted ? 'Unmute Voices 🔊' : 'Mute Voice 🔇';
...
function resetMuteState() {
    isMuted = false;
    document.getElementById('mute-btn').textContent = 'Mute Voice 🔇';
}
// Updated speakDirections function
function speakDirections(text) {
    const utterance = new SpeechSynthesisUtterance(text);
    if (isMuted) {
        synth.cancel(); // Ensure no speech occurs if muted
        return;
    }
...
function routeToDestination(startCoords, endCoords) {
    ...
    routingControl.on('routesfound', function (e) {
        ...
        if (routes.length > 0) {
            ...
            const textSummary = `The route is approximately ${Math.round(summary.totalDistance / 1000)} kilometers and will take about ${Math.round(summary.totalTime / 60)} minutes.`;
            speakDirections(textSummary);
            

            const directionsDiv = document.getElementById('directions');
            directionsDiv.innerHTML = `<p><strong>${textSummary}</strong></p>`;

            
            // Step-by-step directions
            const stepDirections = steps.map((step, index) => {
                return `<p>Step ${index + 1}: ${step.text} (${step.distance} meters)</p>`;
            }).join('');
    
            directionsDiv.innerHTML += stepDirections;
    
            // Speak each step sequentially
            steps.forEach((step, index) => {
                const spokenText = `Step ${index + 1}: ${step.text}, ${step.distance} meters.`;
                setTimeout(() => speakDirections(spokenText), index * 4000); // Delay for speaking steps
            });
        }
    });
...
function handleNavigationUpdate() {
     // Reset mute state when either start or destination changes
    resetMuteState();
